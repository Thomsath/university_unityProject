...
public class LineGraphManager : MonoBehaviour {
	...
	void OnApplicationQuit(){
		...
		string jsontext = "{ \"values\" : [ ";
		for (int i = 0; i < Templist.Count-1; i++) {
			Point a = new Point (System.Convert.ToDouble(ListTime [i]),System.Convert.ToDouble(Templist [i]));
			string pointtojson = JsonUtility.ToJson (a);
			jsontext += pointtojson + ",";
			//UnityEngine.Debug.Log (pointtojson);
		}

		Point b = new Point (System.Convert.ToDouble(ListTime [Templist.Count-1]),System.Convert.ToDouble(Templist [Templist.Count-1]));
		string lastpointjson = JsonUtility.ToJson (b);
		jsontext += lastpointjson ;
		//UnityEngine.Debug.Log (lastpointjson);
		jsontext += "]}";
		UnityEngine.Debug.Log (jsontext);


	
	    /*var httpWebRequest = (HttpWebRequest)WebRequest.Create ("http://info-timide.iut.u-bordeaux.fr/perso/2017/tmansier/index.php");
		...
		File.WriteAllText (path +"\\"+ "datatemp.json" , jsontext);
	}
